...
class ResidualBlock(nn.Module):
    def __init__(self, input_channels, output_channels, stride=1):
        ...
        self.stride = stride
        self.sub4 = int(int(output_channels / 4))
        self.bn1 = nn.BatchNorm2d(input_channels)
        self.relu = nn.ReLU(inplace=True)
        self.conv1 = nn.Conv2d(input_channels, self.sub4, 1, 1, bias=False)
        self.bn2 = nn.BatchNorm2d(self.sub4)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = nn.Conv2d(self.sub4, self.sub4, 3, stride, padding=1, bias=False)
        self.bn3 = nn.BatchNorm2d(self.sub4)
        self.relu = nn.ReLU(inplace=True)
        self.conv3 = nn.Conv2d(self.sub4, output_channels, 1, 1, bias=False)
