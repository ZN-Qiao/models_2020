...
class eca_layer(nn.Module):
    ...
    def forward(self, x):
        ...
        y = self.avg_pool(x)
        print("average size:", y.size())
        print(y.clone().squeeze(-1).size())
        print(y.clone().squeeze(-1).transpose(-1, -2).size())
        print(self.conv(y.clone().squeeze(-1).transpose(-1, -2)).size())
...
def demo():
    ...
    net = eca_resnet50(num_classes=365)
