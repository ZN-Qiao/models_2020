...
class SCConv(nn.Module):
    def __init__(self, inplanes, planes, stride, padding, dilation, groups, pooling_r, norm_layer, level):
        super(SCConv, self).__init__()
        if level == 1:
            self.k2 = nn.AdaptiveAvgPool2d(1)
        elif level == 2:
            self.k2 = nn.AdaptiveAvgPool2d(2)
        elif level == 4:
            self.k2 = nn.AdaptiveAvgPool2d(4)
    ...
    def forward(self, x):
        identity = self.k3(x)
        k2 = self.k2(x)
        out = torch.sigmoid(torch.add(identity, F.interpolate(k2, identity.size()[2:])))  # sigmoid(identity + k2)
...
class SCBottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None,
        ...
        self.scconv = SCConv(
            ...
            padding=dilation, dilation=dilation,
            groups=cardinality, pooling_r=self.pooling_r, norm_layer=norm_layer, level = 2)
...
class SCNet(nn.Module):
    ...
    def __init__(self, block, layers, groups=1, bottleneck_width=32,
                 num_classes=365, dilated=False, dilation=1,
