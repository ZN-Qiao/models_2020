...
class InvertedResidual(nn.Module):
    def __init__(self, inp, oup, stride, expand_ratio):
        ...
        self.sa = ParamFree(oup)

    def forward(self, x):

        if self.use_res_connect:
            return x + self.sa(self.conv(x))
        else:
            return self.sa(self.conv(x))


class MobileNetV2(nn.Module):
    def __init__(self, num_classes=365, width_mult=1.0, inverted_residual_setting=None, round_nearest=8):
        """
...
def demo():
    ...
    net = se_mobilenet_v2(num_classes=365)
