...
class bLModule(nn.Module):
    def __init__(self, block, in_channels, out_channels, blocks, alpha, beta, stride):
        ...
        self.fusion = self._make_layer(block, out_channels*2, out_channels, 1, stride=stride)
    ...
    def forward(self, x):
        ...
        out = self.relu(torch.cat([big, little], dim = 1))
        out = self.fusion(out)
...
class bLResNet(nn.Module):
    ...
    def __init__(self, block, layers, alpha, beta, num_classes=365):
...
def blresnet_model(depth, alpha, beta, num_classes=365, pretrained=False):
    layers = {
        50: [3, 4, 6, 3],
        101: [4, 8, 18, 3],
        152: [5, 12, 30, 3]
...
def demo():
    ...
    net = blresnet_model(50, 2, 4, num_classes=365)
