...
from torch.nn.parameter imporimport torch.nn.functional as F
t Parameter
import torch
...
__all__ = ['update7_resnet18', 'update7_resnet34', 'update7_resnet50', 'update7_resnet101', 'update7_resnet152']
...
class ParamFree(nn.Module):
    ...
    def forward(self, x):
        b, c, h, w = x.size()
        gap = self.batch_norm(F.interpolate(self.gap(x), size=([h, w]))).unsqueeze(dim=1)
        gap2 = self.batch_norm(F.interpolate(self.gap2(x), size=[h, w])).unsqueeze(dim=1)
        gap4 = self.batch_norm(F.interpolate(self.gap4(x), size=[h, w])).unsqueeze(dim=1)
        x = x.unsqueeze(dim=1)

        all_out = torch.cat([x, gap, gap2, gap4], dim=1)
...
def update7_resnet18(pretrained=False, **kwargs):
...
def update7_resnet34(pretrained=False, **kwargs):
...
def update7_resnet50(pretrained=False, **kwargs):
...
def update7_resnet101(pretrained=False, **kwargs):
...
def update7_resnet152(pretrained=False, **kwargs):
...
def demo():
    ...
    for i in range(1):
        net = update7_resnet50(num_classes=1000)
